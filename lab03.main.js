 ...
 * @return {string} (Ipv4MappedIpv6Address) - An IPv6 address.
 */
...
function main() {
  ...
  for (let i = 0; i < sampleCidrsLen; i++) {
    ...
    getFirstIpAddress(sampleCidrs[i], (data,ipv6data, error) => {
      // Now we are inside the callback function.
      // Display the results on the console.
      if (error) {
        console.error(`  Error returned from GET request: ${error}`);
      }
      console.log(`  Response returned from GET request: {"ipv4":${data},"ipv6":${ipv6data}}`);
    });
  }
